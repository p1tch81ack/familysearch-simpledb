...
class SimpleTable[T](baseIterable: Iterable[T], val fieldMapper:FieldMapper[T])(implicit classTag:ClassTag[T] ) extends Table[T]{
  ...
  def getMapOfMatchingRows(matchSpecifier: RowSpecifier): immutable.Map[immutable.Map[String, AnyRef], List[T]] = mainIndex.getMapOfMatchingRows(matchSpecifier)
